...
const Card = ({ project }: ICardProps) => {
  return (
    <div className={`relative rounded-[11px] overflow-hidden w-fit ${style.card}`}>
      <Image
        src={`${project.image}`}
      ...
      <div className={style.info}>
        <div className="ml-[10%] mr-[10%] mb-[13px]">
          ...
          <div className={`${style.boxTechs}`}>
            {project.stack.map((tech) => <strong className={`${style.tech}`} key={crypto.randomUUID()}>{tech}</strong>)}
          </div>
        </div>
      </div>
    </div>
